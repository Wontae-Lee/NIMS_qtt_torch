...
def get_graph_feature(coords, feats, k=10):
    ...
    # [B, N, kernel_size+1], here we ignore the smallest element as it's the query itself
    idx = idx[:, :, 1:]  # [B, N, kernel_size]

    idx = idx.unsqueeze(1).repeat(1, C, 1, 1)  # [B, C, N, kernel_size]
    all_feats = feats.unsqueeze(2).repeat(1, 1, N, 1)  # [B, C, N, N]

    neighbor_feats = torch.gather(all_feats, dim=-1, index=idx)  # [B, C, N, kernel_size]
